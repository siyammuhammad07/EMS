...
public class MyHashTable {
	...
	public void displayContents() {
        ...
        public EmployeeInfo removeEmployee(int theEmpNum) {
            ...
            int number = calcBucket(theEmpNum); 
		for (int i = 0; i < buckets[number].size(); i++) {
			int currentNum = buckets[number].get(i).empNum;
			if (currentNum == theEmpNum) { 
                            return buckets[number].remove(i);				 
			}
		}
            return null;
        }
		...
		for (int i = 0; i < buckets[number].size(); i++) {
			int currentNum = buckets[number].get(i).empNum;
			if (currentNum == theEmpNum) { 
				EmployeeInfo employee = buckets[number].get(i);
				return employee;
			}
		}
